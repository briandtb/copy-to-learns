...
func (sm *Mutex) Lock(ctx context.Context) {
	...
	if waiting := len(sm.waiters); waiting > 0 && activeSpan != nil {
		activeSpan.LogFields(
			...
			log.String("blockers", fmt.Sprintf("%v", sm.waiters)),
		) // avoid deferred slice.String()
	...
	if activeSpan != nil {
		activeSpan.LogFields(
			log.String("event", fmt.Sprintf("Acquired lock with %d transactions waiting behind", behindLen)))
	}
}
