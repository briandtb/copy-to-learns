...
func unmountNetworkNamespace(containerID string) {
	...
	if err := syscall.Unmount(netNsPath, 0); err != nil {
		log.Fatal("Unable to mount container file system: %v at %s", err, netNsPath)
	}
}
...
func execContainerCommand(mem int, swap int, pids int, cpus float64,
	...
	doOrDieWithMsg(copyNameserverConfig(containerID), "Unable copy resolve.conf")
	...
	doOrDieWithMsg(syscall.Mount("sysfs", "/sys", "sysfs", 0, ""), "Unable to mount devpts")
	setupLocalInterface()
...
func prepareAndExecuteContainer(mem int, swap int, pids int, cpus float64,
	...
	/*
		From namespace(7)
			...
			Network 	CLONE_NEWNET	Network device,
										stacks, ports, etc.
