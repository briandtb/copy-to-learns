...
/*
	...
	error. As a workaroung, we loop through the interfaces, compare the name,
	set the IP and make the interface ip.
*/
...
func setupNewNetworkNamespace(containerID string) {
	...
	if err := syscall.Unshare(syscall.CLONE_NEWNET); err != nil {
		log.Fatalf("Unshare syscall call failed: %v\n", err)
	}
