...
func createCGroups(containerID string, createCGroupDirs bool) {
	...
	for _, cgroupDir := range cgroups {
		...
		doOrDieWithMsg(ioutil.WriteFile(cgroupDir+"/cgroup.procs",
			[]byte(strconv.Itoa(os.Getpid())), 0700), "Unable to write cgroup procs file")
	}
}
...
func setMemoryLimit(containerID string, limitMB int, swapLimitInMB int) {
	...
	doOrDieWithMsg(ioutil.WriteFile(memFilePath, []byte(strconv.Itoa(limitMB*1024*1024)), 0644),
	...
	/*
		...
		If memory.limit_in_bytes is specified but memory.memsw.limit_in_bytes
		is left untouch , processes in the control group will continue to
		consume swap space.
	*/
	if swapLimitInMB >= 0 {
...
func setCPULimit(containerID string, limit float64) {
	...
	if limit > float64(runtime.NumCPU()) {
		fmt.Printf("Ignoring aptempt to set CPU quota to great than number of available CPUs")
		return
	}

	doOrDieWithMsg(ioutil.WriteFile(cfsPeriodPath,
		[]byte(strconv.Itoa(1_000_000)), 0644),
		"Unable to write CFS period")

	doOrDieWithMsg(ioutil.WriteFile(cfsQuotaPath,
		[]byte(strconv.Itoa(int(1_000_000*limit))), 0644),
		"Unable to write CFS period")
}
