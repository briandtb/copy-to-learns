...
var _ = Describe("In-memory storage", func() {
	...
	It("should not error out if duplicate nodes wre received, with a partial store", func() {
	...
	It("should no clear storage cache when input is empty batch", func() {
		...
		By("ensuring the storage previous cache for pods remains")
		for _, pod := range batch.Pods {
			ts, metrics := storage.GetContainerMetrics(apitypes.NamespacedName{
				Name:      pod.Name,
				Namespace: pod.Namespace,
	...
	It("should return nil metrics for missing pods", func() {
		...
		By("fetching a present pod and a missing pod")
		ts, containerMetrics := storage.GetContainerMetrics(apitypes.NamespacedName{
			Name:      "pod1",
			Namespace: "ns1",
		...
		Expect(ts).To(Equal([]api.TimeInfo{
			{Timestamp: now.Add(400 * time.Millisecond), Window: defaultWindow},
			{},
		}))
	...
	It("should retrieve metrics for a node, with overall latest scrape time", func() {
		...
		By("verifying that the timestamp is the smallest time amongst all containers")
		Expect(ts).To(Equal([]api.TimeInfo{
			{Timestamp: now.Add(100 * time.Millisecond), Window: defaultWindow},
			{Timestamp: now.Add(200 * time.Millisecond), Window: defaultWindow},
		}))
